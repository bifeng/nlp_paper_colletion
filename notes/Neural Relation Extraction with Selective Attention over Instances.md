​	关系抽取是实体识别基础上的一个任务，其核心是抽取一个句子中包含实体对之间的关系。目前关系抽取中比较常见的一类做法是远程监督关系抽取方法，然而该方法会引入大量的噪声数据，从而影响关系抽取的性能。本方案采用PCNN+attention模型，该模型通过CNN/PCNN对句子进行编码，然后利用attention对句子进行筛选，可以有效降低噪音数据的影响。
