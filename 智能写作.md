...
抽取式主要从文档中抽取已有句子形成摘要。该方法可以保证句子的可读性。

其核心还是对句子做ranking和resort。

典型框架：

![extractive_architect](https://github.com/bifeng/nlp_paper_notes/raw/master/image/extractive_architect.png)
...
“结构化数据写作”，主要将结构化数据转化为文本。应用于天气、股票、体育等结构化数据场景；

“新闻摘要”，基于深度语义分析模型，自动抽取新闻文本中的关键信息并生成指定长度的新闻摘要。可用于热点新闻聚合、新闻推荐、语音播报、APP消息Push等场景。应用案例：百家号。
